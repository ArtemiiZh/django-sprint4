...
def post_detail(request, post_id):
    ...
    # Для детальной страницы автор должен видеть свой пост, даже если он скрыт
    post = get_object_or_404(
        Post.objects.select_related('category', 'location', 'author').annotate(comment_count=Count('comments')),
        pk=post_id
    )
    # Если пост скрыт и зашел не автор — отдаем 404
...
def category_posts(request, category_slug):
    ...
    posts_list = get_published_posts().filter(category=category).order_by('-pub_date')
    page_obj = paginate_queryset(request, posts_list)
...
def profile(request, username):
    ...
    posts_list = Post.objects.select_related('category', 'location', 'author').annotate(comment_count=Count('comments')).filter(author=profile_user)
...
def post_edit(request, post_id):
    ...
    # Если зашёл не автор, перенаправляем на страницу просмотра этого поста
...
def post_delete(request, post_id):
    ...
    if request.method == 'POST':
        post.delete()
        return redirect('blog:profile', username=request.user.username)
    # По ТЗ переиспользуем существующий шаблон подтверждения удаления
